...
i_url ="https://www.instagram.com/hackers_colony_official?igsh=Y2NjMXQxZTlmMXho"
...
os.system('clear')
writer(sub_hading)
writer("""\033[1;32mThis Python-based tool cracks zip file passwords using the brute force method. In brute force, the tool systematically tries all possible password combinations. You can specify the file path, minimum password length, and maximum password length. The default minimum password length is 0 and the default maximum password length is 9999999.

Key Features:

File Path Input: Enter the full path of the zip file you want to crack.

Customizable Password Length: Set the minimum (default 0) and maximum (default 9999999) password length. The tool will try all combinations within the specified length range.

Comprehensive Password Generation: It generates all possible combinations of characters—letters, numbers, and special characters.

Automated Process: The password recovery process is fully automated—just provide the file path and length range, and the tool handles the rest.

Customizable Speed and Parameters: You can adjust the attack speed and other parameters.
